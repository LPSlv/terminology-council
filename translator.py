...
class Translator:
    ...
    def _load_with_sharding(self):
        ...
        try:
            ...
            print(f"  Pipeline config: tokenizer.model_max_length={self.tokenizer.model_max_length}, "
    ...
    def _load_single_device(self, use_gpu: bool):
        ...
        try:
            ...
            num_gpus = torch.cuda.device_count()
            if num_gpus > 1:
                print(f"  MT load OOM on single GPU; retrying with device_map=auto across {num_gpus} GPUs")
                self.use_sharding = True
                self._load_with_sharding()
            else:
                raise RuntimeError(f"MT model OOM on single GPU: {e}")
        except Exception as e:
            import traceback
            print(f"ERROR loading MT model: {e}")
            traceback.print_exc()
            raise RuntimeError(f"Failed to load MT model {self.model_id}: {e}")
    ...
    def translate(self, source_text: str) -> str:
        ...
        try:
            encoded = self.tokenizer(source_text, return_tensors="pt", truncation=True, max_length=1024)
