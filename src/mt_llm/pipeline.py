...
class Pipeline:
    ...
    def load_models(self):
        ...
        if self.translator and self.translator.tokenizer:
            ...
            print(
                f"Generation kwargs: max_new_tokens={self.max_new_tokens}, truncation=True, tokenizer_max_length={max_input_len}"
            )
        else:
            print(f"Generation kwargs: max_new_tokens={self.max_new_tokens}, truncation=True")

    def run(
        self, source_text: str, terms: dict[str, str] | None = None, skip_checker: bool = False
    ) -> dict:
    ...
    def run_batch(
        ...
        if not skip_checker and self.checker is not None:
            ...
            for i, (checker_result, terms) in enumerate(
                zip(checker_results, terms_list, strict=False)
            ):
        ...
        # Build results
        results = []
        for i, (source_text, mt_translation, final_translation) in enumerate(
            zip(source_texts, mt_translations, final_translations, strict=False)
        ):
