...
class Checker:
    ...
    def _load_model(self):
        try:
            ...
            traceback.print_exc()
            sys.stdout.flush()
            raise RuntimeError(f"Failed to load checker model {self.model_id}: {e}") from e
    ...
    def check(
        ...
        if is_junk:
            if used_checker:
                print(
                    f"  [Checker] Output detected as junk (MT_hash={mt_hash}, checker_hash={checker_hash}), using MT fallback"
                )
        ...
        terminology_issues = (
            self._check_terminology(final_translation, terms)
            if (self.terminology_mode == "on" and terms)
            else []
        )
    ...
    def _generate(self, prompt: str, max_new_tokens: int) -> str:
        ...
        if is_chat_model and hasattr(self.tokenizer, "apply_chat_template"):
            ...
            inputs = self.tokenizer(
                formatted_prompt, return_tensors="pt", truncation=True, max_length=max_input_len
            )
        else:
            # Calculate max input length: model_max_length - max_new_tokens - safety margin
            max_input_len = self.tokenizer.model_max_length - max_new_tokens - 10
        ...
        generated_text = re.sub(
            r"^(Assistant|User|System):\s*", "", generated_text, flags=re.IGNORECASE
        )
        ...
        for inputs in tokenized_inputs:
            ...
            if padding_length > 0:
                padded_input_ids = torch.cat(
                    [torch.full((padding_length,), pad_token_id, device=device), input_ids]
                )
                padded_attention_mask = torch.cat(
                    [torch.zeros((padding_length,), device=device), attention_mask]
                )
            else:
    ...
    def check_batch(
        ...
        prompt_list = []
        for source_text, mt_translation, terms in zip(
            source_texts, mt_translations, terms_list, strict=False
        ):
            prompt_terms = terms if self.terminology_mode == "on" else None
            prompt_memory = memory if self.terminology_mode == "on" else None
            prompt = get_checker_prompt(
        ...
        junk_count = 0
        applied_count = 0
        for i, (source_text, mt_translation, terms, raw_output) in enumerate(
            zip(source_texts, mt_translations, terms_list, raw_outputs, strict=False)
        ):
            ...
            if is_junk:
                if used_checker:
                    ...
                    if i < 2:  # Log first 2 junk cases for debugging
                        print(
                            f"  [Checker] Junk detected: raw_hash={raw_hash}, extracted_hash={checker_hash}, mt_hash={mt_hash}"
                        )
                final_translation = mt_translation
            elif used_checker:
                applied_count += 1
                if i < 2:  # Log first 2 applied cases for debugging
                    print(
                        f"  [Checker] Applied: raw_hash={raw_hash}, extracted_hash={checker_hash}, mt_hash={mt_hash}"
                    )
        ...
        if junk_count > 0 or applied_count > 0:
            print(
                f"  [Checker batch] Applied={applied_count}, Junk={junk_count}, Unchanged={len(source_texts) - applied_count - junk_count}/{len(source_texts)}"
            )
    ...
    def _is_junk_output(self, output: str, mt_output: str, source_text: str) -> bool:
        ...
        if (
            len(common_english_words) > len(source_words) * 0.5
            and len(output) > len(mt_output) * 1.5
        ):
            return True
    ...
    def _extract_translation(self, output: str, fallback: str) -> str:
        ...
        cleaned = re.sub(
            r"^(Translation|German|Corrected|Output|Corrected German translation):\s*",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
        cleaned = re.sub(
            r"\s*(Translation|German|Corrected|Output)\.?\s*$", "", cleaned, flags=re.IGNORECASE
        )
        ...
        for pattern in prompt_markers:
            ...
            if match:
                ...
                extracted = re.split(
                    r"(?:Review|Your tasks|English:|Corrected German translation:)",
                    extracted,
                    flags=re.IGNORECASE,
                )[0].strip()
        ...
        for line in lines:
            ...
            if any(
                word in line.lower()
                for word in ["review", "translation", "quality", "accuracy", "this is", "here is"]
            ):
    ...
    def _detect_changes(self, original: str, corrected: str) -> list[dict[str, str]]:
        ...
        if original.strip() != corrected.strip():
            changes.append(
                {"from": original, "to": corrected, "reason": "Translation improved by checker"}
            )
        return changes
    ...
    def _check_terminology(self, translation: str, terms: dict[str, str]) -> list[dict[str, str]]:
        ...
        for en_term, expected_de_term in terms.items():
            ...
            if not re.search(pattern, translation_lower):
                issues.append(
                    {"source_term": en_term, "expected": expected_de_term, "found": "not found"}
                )
